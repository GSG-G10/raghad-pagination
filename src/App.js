...
import { Pagination, Image, Spin, message } from "antd";
...
function App() {
  ...
  const antIcon = (
    <LoadingOutlined style={{ fontSize: 24, margin: "15vh 0" }} spin />
  );
    ...
    fetch(
      ...
      .then((data) => {
        const newArr = data.results.map((elem) => {
          return elem.cover_photo.urls.full;
        });
        setImgUrl(newArr);
      })
  ...
  return (
    <>
      <h1 className="site-page-header">Cats World</h1>
      <Pagination
